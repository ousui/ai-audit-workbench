...
def build_record(seq: int, decision: str, triage: dict[str, Any], candidate: dict[str, Any]) -> dict[str, Any]:
    prefix = decision if decision != "FIND" else "FIND"
    risk_id = f"{prefix}-{seq:05d}"
    return {
        "risk_id": risk_id,
        "decision": decision,
...
def merge(run_root: Path) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "schema_version": "merge-result-0.1.0",
        "run": pack.get("run"),
        "project": {
        ...
        "summary": {
            ...
            "fp_count": 0,
            "blocked_count": 0,
            "report_include_count": 0,
            "unknown_candidate_refs": 0,
        },
        "findings": [],
        "review_items": [],
        "runtime_items": [],
    ...
    result["summary"].update({
        "find_count": len(result["findings"]),
        "review_count": len(result["review_items"]),
        "runtime_count": len(result["runtime_items"]),
        "candidate_count": len(result["candidate_items"]),
        "no_risk_count": len(result["no_risk_items"]),
        "fp_count": len(result["fp_items"]),
        "blocked_count": len(result["blocked_items"]),
        "report_include_count": len(result["findings"]) + len(result["review_items"]) + len(result["runtime_items"]) + len(result["blocked_items"]),
    })
...
def render_md(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- RUNTIME: {s['runtime_count']}",
        f"- CAND: {s['candidate_count']}",
        f"- NO_RISK: {s['no_risk_count']}",
        f"- FP: {s['fp_count']}",
        f"- BLOCKED: {s['blocked_count']}", "",
    ]
    for key, title in [("findings", "FIND"), ("review_items", "REVIEW"), ("runtime_items", "RUNTIME"), ("candidate_items", "CAND")]:
        lines.extend([f"## {title}", ""])
        items = result.get(key) or []
        if not items:
            lines.append("- None")
        for item in items[:80]:
            lines.append(f"- `{item['risk_id']}` `{item.get('file_path')}:{item.get('line_start')}` {item.get('title')}")
        lines.append("")
    if result.get("notes"):
        lines.extend(["## Notes", ""])
